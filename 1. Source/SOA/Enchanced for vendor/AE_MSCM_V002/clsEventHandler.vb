                    ...
                    Select Case pVal.MenuUID
                        ...
                        Case "FSOA"
                            Try
                                ...
                                ' ''F_SOA = New Form1
                                ' ''F_SOA.Show() 
                                If p_iDebugMode = DEBUG_ON Then Call WriteToLogFile_Debug("LoadFromXML", "MenuEvent()")
                    ...
                    Select Case pVal.FormUID
                        Case "SOA"
                            ...
                            If pVal.EventType = SAPbouiCOM.BoEventTypes.et_COMBO_SELECT Then
                                If pVal.ItemUID = "t_type" And pVal.ItemChanged = True Then
                                    Dim oForm As SAPbouiCOM.Form = p_oSBOApplication.Forms.ActiveForm
                                    Try
                                        Dim Type As SAPbouiCOM.ComboBox = oForm.Items.Item("t_type").Specific
                                        Dim oMatrix As SAPbouiCOM.Matrix = oForm.Items.Item("Item_8").Specific

                                        Dim SelectType As String = Type.Selected.Value
                                        oForm.Items.Item("Item_1").Specific.value = ""
                                        oForm.Items.Item("BPFrom").Specific.value = ""
                                        oForm.Items.Item("Item_3").Specific.value = ""
                                        oForm.Items.Item("BPTo").Specific.value = ""
                                        oMatrix.Clear()
                                        oForm.ActiveItem = "BPFrom"
                                        If Set_Conditions(oForm, sErrDesc) <> RTN_SUCCESS Then Throw New ArgumentException(sErrDesc)

                                    Catch ex As Exception
                                        p_oSBOApplication.SetStatusBarMessage("Combo select Event Failed", SAPbouiCOM.BoMessageTime.bmt_Medium, True)
                                    End Try
                                End If
                            End If
                                        ...
                                        For mjs As Integer = 1 To oMatrix.RowCount
                                            ...
                                            If oCheck.Checked And oMatrix.Columns.Item("Col_5").Cells.Item(mjs).Specific.String <> "Successfully Sent Email" Then
                                                ...
                                                If ExportToPDF(oMatrix.Columns.Item("Col_1").Cells.Item(mjs).Specific.String, oMatrix.Columns.Item("Col_1").Cells.Item(mjs).Specific.String, _
                                                              System.DateTime.Parse(oForm.Items.Item("Item_7").Specific.String, format1, Globalization.DateTimeStyles.None), "2", sTargetFileName, sRptFileName, SBO_Application, p_oDICompany, sErrDesc, oForm.Items.Item("t_type").Specific.value.ToString) <> RTN_SUCCESS Then
